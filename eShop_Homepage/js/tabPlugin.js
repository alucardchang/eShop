...
~function () {
    class Tab {
        ...
        init() {
            ...
            this.offsetT = $(this.container).offset().top;
            this.instAry=[];
        ...
        bindData(data, page) {
            ...
            for (let i = 0; i < data[page].length; i++) {
                ...
                str += `<div class="goods">
                    <a href="javascript"><img data-src="${img}"></a>
                    <p><a href="javascript">${title}</a></p>
                    <span>￥${price}</span>
                    <a href="javascript"></a>
                </div>`
            }
            /*Only one con div needs to update data*/
            this.conList[0].innerHTML = str;
            this.preLoad(page);
        }

        /*Image Preload*/
        preLoad(page) {
            let $imgEles = $(this.container).find('.con').find('img');
            if (this['prodData'][page].isLoaded) {
                console.log("ok");
                for (let i = 0; i < $imgEles.length; i++) {
                    let $curImg = $imgEles.eq(i),
                        imgSrc = $curImg.attr('data-src');
                    $curImg.css({'display': 'block'}).attr('src', imgSrc);
                }
                return;
            }
            window.onscroll.call(this,function(){
                console.log(this);
            });

            for (let i = 0; i < $imgEles.length; i++) {
                let $curImg = $imgEles.eq(i);
                let imgSrc = $curImg.attr('data-src'),
                    fakeImg = new Image();
                fakeImg.onload = function () {
                    $curImg.attr('src', imgSrc).stop().fadeIn(2000);
                    fakeImg = null;
                }
                fakeImg.src = imgSrc;
            ...
            //Prevent image preloading one more time
            this['prodData'][page].isLoaded = true;
        }
